...
# Subagent Delegation
Keep the main context lean. Delegate multi-file exploration, review, planning, research, and second-opinion work to subagents instead of doing it inline. Run `{ action: "list" }` first when unsure which agents are available.
...
| Independent subtasks (review, test, complexity) | `{ tasks: [...] }` | Run concurrently when no dependencies |
